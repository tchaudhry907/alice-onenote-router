// /lib/msgraph.v6.js
// V6 Graph client used by the cron drain. Side-by-side with your legacy lib/msgraph.js.
// - Reads Graph access token from KV (no refresh here).
// - Finds/creates a target page by pageKey and appends HTML; or creates a new page.
...
async function ensurePage(sectionId, pageKey, title) {
  try {
    const data = await graphFetch(
      "GET",
      `/me/onenote/sections/${encodeURIComponent(sectionId)}/pages?$search=${encodeURIComponent(pageKey)}`
    );
  ...
  const seedHtml = `
    <html>
      <head><title>${escapeHtml(title || pageKey)}</title></head>
      <body>
...
async function createPage(sectionId, title, innerHtml) {
  const fullHtml = `
    <html>
      <head><title>${escapeHtml(title || "Log")}</title></head>
      <body>${innerHtml}</body>
    </html>
  `.trim();
  const created = await graphFetch(
