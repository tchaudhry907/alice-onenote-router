// KV_FILE_VERSION: v9-2025-09-17
// lib/kv.js — Upstash Redis REST client (uses proper REST paths; supports all import styles)
...
export async function set(key, value, opts = {}) {
  // Upstash expects value in the PATH, not in JSON body
...
export async function del(key) {
  // Use GET /del/{key} (Upstash accepts this)
  const r = await http('GET', `/del/${encodeURIComponent(key)}`);
  // result may be integer count; coerce to number
