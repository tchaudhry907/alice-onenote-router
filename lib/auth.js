// lib/auth.js
// Delegated (device code) auth for Microsoft Graph.
// Stores refresh_token in Upstash (or memory fallback).
...
const AT_TTL_SAFETY = 20; // seconds trimmed off reported expires_in
...
export async function beginDeviceFlow() {
  ...
  const r = await fetch(`${BASE}/devicecode`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: params
  });
  ...
  // Cache device_code for later poll (short TTL)
...
export async function pollDeviceFlow() {
  ...
  try {
    ...
    // Store tokens
    await kvSet(RT_KEY, data.refresh_token, 60 * 60 * 24 * 90); // 90 days
...
export async function getAccessToken() {
  ...
  // 3) No tokens → require device flow first
  const e = new Error('AUTH_REQUIRED');
  e.auth_required = true;
  throw e;
}

// convenience for manual reset
