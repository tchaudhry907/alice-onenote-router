// lib/auth.js
// Uses your MS_* environment variables directly to mint a Microsoft Graph token.
// Also exports requireAuth so older routes keep building.

export async function getAccessToken() {
  const tenant = process.env.MS_TENANT || 'common';
  const clientId = process.env.MS_CLIENT_ID;
  const clientSecret = process.env.MS_CLIENT_SECRET;
  const scope = process.env.MS_SCOPES || 'https://graph.microsoft.com/.default';
...
// Back-compat for any older routes importing this:
