// lib/auth-device.js
// Minimal device-code flow against Microsoft identity platform.
// Uses Upstash KV via your existing lib/kv.js.
...
const SCOPES =
  ...
  "offline_access openid profile Notes.ReadWrite.All";

// Keys in KV are namespaced per bearer so multiple users can use the same router.
...
export async function beginDeviceFlow({ bearer }) {
  ...
  // Persist the device flow info to KV for this bearer
...
export async function pollDeviceFlow({ bearer }) {
  ...
  try {
    ...
    // Store tokens (access + refresh) for this bearer
