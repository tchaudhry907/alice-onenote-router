...
export default async function handler(req, res) {
  ...
  // Accept either header or session cookie
  const bearer =
  ...
  const { notebookName = "AliceChatGPT", sectionName = "Hobbies",
          title = "[DIAG] Test page", html = "<p>Hello from Diagnostics</p>" } = req.body || {};
  ...
  try {
    ...
    const section = (secJson.value || []).find(s => (s.displayName || "").trim().toLowerCase() === sectionName.toLowerCase());
