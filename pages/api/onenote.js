...
function buildTargetFromReq(req) {
  ...
  if (req.method === "POST") {
    ...
    let target = url ? url : `${GRAPH_ORIGIN}/v1.0${path.startsWith("/") ? path : `/${path}`}`;
  ...
  throw new Error("Unsupported method; use GET ?url=... or POST { url|path }");
}

function assertAllowed(url) {
  const u = new URL(url);
  const full = `${u.origin}${u.pathname}`;
  if (!full.startsWith(ALLOWED_PREFIX)) throw new Error("Blocked: URL must start with /v1.0/me/onenote");
}
...
export default async function handler(req, res) {
  try {
    ...
    if (!refreshToken) return res.status(401).json({ ok: false, error: "Session expired. Please sign in again." });
