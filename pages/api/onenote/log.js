...
async function appendToPage(accessToken, pageId, htmlFragment) {
  // Must be multipart/related with a part NAMED "commands"
...
export default requireAuth(async function handler(req, res, session) {
  ...
  try {
    ...
    // Inbox section: env first, then constant, then your known fallback
    const sectionId =
    ...
    // Try KV cache (reuse same page for the day)
    let pageId = await kvGet(kvKey);

    // Create the page if not present
    if (!pageId) {
