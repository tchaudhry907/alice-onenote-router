...
// ----- Graph helpers -----
function userPrefix() {
  const upn = process.env.MS_USER_UPN; // REQUIRED for app-only
...
async function getNotebookIdByName(name) {
  const data = await graphFetch(`${userPrefix()}/onenote/notebooks?$select=id,displayName`);
...
async function getSectionIdByName(notebookId, name) {
  const data = await graphFetch(`${userPrefix()}/onenote/notebooks/${encodeURIComponent(notebookId)}/sections?$select=id,displayName`);
...
async function createOneNotePage({ notebookName, sectionName, title, html }) {
  ...
  const created = await graphFetch(
    `${userPrefix()}/onenote/sections/${encodeURIComponent(secId)}/pages`,
  ...
  if (!links.oneNoteWebUrl || !links.oneNoteClientUrl) {
    const p = await graphFetch(`${userPrefix()}/onenote/pages/${encodeURIComponent(pageId)}?$select=id,links`);
...
async function readPageFirstLines(pageId) {
  const html = await graphFetch(`${userPrefix()}/onenote/pages/${encodeURIComponent(pageId)}/content?includeIDs=true`, {
