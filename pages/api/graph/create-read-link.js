...
async function graphFetch(path, { method='GET', headers={}, body } = {}) {
  ...
  if (!r.ok) { const text = await r.text().catch(()=>''); const e=new Error(`Graph ${method} ${path} -> ${r.status}`); e.status=r.status; e.body=text; throw e; }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(status).json({ ok:false, error: err?.body || err?.message || 'Internal Error' });
  }
}
